...
def register_callbacks(app, wales_df, wales_df_long):
    ...
    # Built once at startup, passed into logic functions as needed.
    # ─────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────
    # Callbacks — pure wiring, no logic
    # ─────────────────────────────────────────────────────────────
    ...
    def update_year(sites, pollutant, current_years):
        # Delegates entirely to logic.py
    ...
    def exceedance_bar(selected_sites, pollutant, selected_years, threshold_standard):
        # All the chart-building logic now lives in logic.py:
        # build_exceedance_bar(exceedance_data, selected_sites, pollutant,
        #                      selected_years, threshold_standard, LIMITS)
        #
        # Shown here unrefactored so you can see the before/after contrast.
        # Move it to logic.py the same way the other functions were moved.
        ...
        colours = [
            "red" if e == "Above" else "green" if e == "Within" else "grey"
            for e in results_data["exceeds"]
        ]
        ...
        y_label = (pollutant_labels_who if who_toggle else pollutant_labels_uk).get(
            pollutant, "Value"
        )
    ...
    def highlight_nav(pathname):
        ...
        return nav_class("/"), nav_class("/comparison"), nav_class("/exceedance")
    ...
    def update_date_bounds(store, dq):
        ...
        # compute_allowed_bounds lives in logic.py — fully testable
        min_allowed, max_allowed = compute_allowed_bounds(
            sites,
            pollutant,
            pol_to_dates,
            site_to_dates,
    ...
    def manage_date_selection(
        ...
        if triggered == "last_week":
            start = today - timedelta(days=7)
            end = min(today, RATIFIED_CUTOFF.date()) if dq == "Ratified" else today
            return start, end

        if triggered == "last_month":
            start = today - timedelta(days=30)
            end = min(today, RATIFIED_CUTOFF.date()) if dq == "Ratified" else today
            return start, end

        # compute_allowed_bounds is now testable outside Dash
        min_allowed, max_allowed = compute_allowed_bounds(
            sites,
            pollutant,
            pol_to_dates,
    ...
    def update_summary_stats(sites, pollutant, start_date, end_date, dq):
        ...
        start_dt, end_dt = apply_dq_cap(start_dt, end_dt, dq, RATIFIED_CUTOFF)
    ...
    def update_completeness(sites, pollutant, start_date, end_date, dq):
        ...
        bars = [
            html.Div(
                ...
                children=[
                    html.Div(r["site"], className="completeness-label"),
                    html.Div(
                        className="completeness-bar-track",
                        children=[
                            html.Div(
                                className=f"completeness-bar-fill status-{r['status']}",
                                style={"width": f"{r['completeness']}%"},
                            )
                        ],
                    ),
                    html.Div(
                        f"{r['completeness']}%", className="completeness-percentage"
                    ),
                ],
            )
    ...
    def update_trends(
        ...
        # ── 1. Cap end date for ratified mode ──
        ...
        days = get_days(start_date, effective_end_date)  # logic.py

        # ── 2. No date selected yet ──
        if days is None:
            empty_fig = _empty_figure("Select a date range to begin")
            return (
                _make_kpi("Average", "--", "Awaiting filters"),
                _make_kpi("Peak", "--", "Awaiting filters"),
                _make_kpi("Exceedances", "--", "Awaiting filters"),
                _make_kpi("Variability", "--", "Awaiting filters"),
                "",
                {"display": "none"},
                dcc.Graph(
                    figure=empty_fig,
        ...
        if dff.empty or not selected_pollutant:
            empty_fig = _empty_figure("No data available for selected filters")
            return (
                _make_kpi("Average", "--", "No data"),
                _make_kpi("Peak", "--", "No data"),
                _make_kpi("Exceedances", "--", "No data"),
                _make_kpi("Variability", "--", "No data"),
        ...
        # ── 4. Resolve labels and threshold ──
        ...
        threshold_info = get_threshold_info(selected_pollutant, selected_standard)
        ...
        # ── 5. Build the chart (all chart builders live in logic.py) ──
        if tab == "overview":
        ...
        site_exceedance = get_site_exceedance_summary(  # logic.py
            dff,
            selected_pollutant,
            selected_standard,
        )
        ...
        if site_count == 1:
            ...
            # threshold_comparison_subtitle now returns a dict from logic.py
            avg_subtitle = threshold_comparison_subtitle(
                avg_value, threshold_value, threshold_metric, selected_standard
            )
            max_subtitle = threshold_comparison_subtitle(
                max_value, threshold_value, threshold_metric, selected_standard
            )

            avg_kpi = _make_kpi(
                "Average", format_with_units(avg_value), avg_subtitle["text"]
            )
            max_kpi = _make_kpi(
                "Peak", format_with_units(max_value), max_subtitle["text"]
            )
            exceed_kpi = _make_kpi(
                "Exceedance", f"{exceedance_info['value']}", exceedance_info["label"]
            )
            var_kpi = _make_kpi(
                "Variability", format_with_units(std_value), "Standard deviation"
            )
        else:
            avg_kpi = _make_kpi(
                "Average",
                ...
                format_site_threshold_comparison_lines(
                    site_avg, threshold_value, threshold_metric, selected_standard
                ),
            )
            max_kpi = _make_kpi(
                "Peak",
                ...
                format_site_threshold_comparison_lines(
                    site_max, threshold_value, threshold_metric, selected_standard
                ),
            )
            exceed_kpi = _make_kpi(
                "Exceedance",
        ...
        # ── 8. Build insight text ──
        if mode == "day":
            insight = (
                f"This selection focuses on a very short window. Average {pollutant_label} is {avg_val:.2f} µg/m³, "
                f"with a peak of {max_val:.2f} on {peak_time}. At this range, local variation and unusual values "
                f"matter more than seasonal interpretation."
            )
        elif mode == "short":
            insight = (
    ...
    def update_quick_btn_active(yday, last_week, last_month, reset, start_date):
        triggered = callback_context.triggered_id
        base_cls, active_cls = "quick-date-btn", "quick-date-btn active"
...
# ─────────────────────────────────────────────────────────────
# Private UI helpers — only used inside callbacks above.
# These are NOT in logic.py because they return Dash html components.
# They are NOT tested because they contain no logic, only layout.
# ─────────────────────────────────────────────────────────────
...
def _make_kpi(title, value, subtitle):
    return html.Div(
        [
...
def _insight_box(text: str):
    return html.Div(
        html.P([html.Strong("Insights: "), text], className="insight-inline"),

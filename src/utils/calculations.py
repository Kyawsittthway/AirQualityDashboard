...
def exceedance_summary(df):
    ...
    grouped = df.dropna(subset=['value']).groupby(
        ['site', 'year', 'pollutants'])
    for (site, year, pollutant), wales_data in grouped:
        ...
        if pollutant in ['PM2.5', 'PM10', 'NO2']:
            ...
            wales_data['8h_mean'] = wales_data['value'].rolling(
                '8h', min_periods=6).mean()
            wales_data = wales_data.reset_index()
            daily_max = (wales_data.groupby(wales_data['date'].dt.date)[
                         '8h_mean'].max().reset_index())
            daily_max['date'] = pd.to_datetime(daily_max['date'])
            daily_max['month'] = daily_max['date'].dt.to_period('M')
            monthly_mean = (daily_max.groupby('month')[
                            '8h_mean'].mean().reset_index())
            monthly_mean = monthly_mean.sort_values('month')
            monthly_mean['6m'] = (monthly_mean['8h_mean'].rolling(
                window=6, min_periods=6).mean())
            value = monthly_mean['6m'].max()
            who_limit = LIMITS['WHO']['O3']['peak']

        elif pollutant == 'SO2':
            # use number of days exceeding the who limits
            daily_mean = wales_data.groupby(wales_data['date'].dt.date)[
                'value'].mean()
